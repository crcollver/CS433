...
void FCFSScheduler::add_to_ready_queue(PCB* pcb, int current_time)
{
	pcb->rq_arrival = current_time; // mark arrival time into readyqueue
	readyqueue.push(pcb); // add it to readyqueue
}
...
PCB* FCFSScheduler::pop_ready_queue(int current_time)
{
	PCB* pcb = readyqueue.front(); // store first element
	pcb->rq_leave = current_time; // mark when PCB leaves readyqueue
...
void FCFSScheduler::schedule(int current_time)
{
